...
import { api, type UmbrelApp } from '../../api/client';
...
export default function UmbrelStore() {
  ...
  const { data: registriesData, isLoading: registriesLoading } = useQuery({
    queryKey: ['umbrelRegistries'],
    queryFn: () => api.getUmbrelRegistries(),
  });
  ...
  const syncMutation = useMutation({
    mutationFn: (registryId: string) => api.syncApps(registryId),
    onSuccess: (data) => {
      showSuccess(data.message || `Synced ${data.synced} apps`);
      queryClient.invalidateQueries({ queryKey: ['apps'] });
      queryClient.invalidateQueries({ queryKey: ['umbrelRegistries'] });
    },
    onError: (err) => {
  ...
  const addRegistryMutation = useMutation({
    mutationFn: ({ id, name, url }: { id: string; name: string; url: string }) =>
      api.addUmbrelRegistry(id, name, url),
    onSuccess: () => {
  ...
  const updateRegistryMutation = useMutation({
    mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) =>
      api.updateUmbrelRegistry(id, { enabled }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['umbrelRegistries'] }),
    onError: (err) => showError(err instanceof Error ? err.message : 'Failed to update registry'),
  });

  const removeRegistryMutation = useMutation({
    mutationFn: (id: string) => api.removeUmbrelRegistry(id),
    onSuccess: () => {
  ...
  // Normalize Umbrel app to common format
  const normalizeApp = (app: UmbrelApp): NormalizedApp => ({
    id: app.id,
    name: app.name,
    version: app.version,
    tagline: app.tagline || '',
    ...
    icon: app.icon || '',
    port: app.port,
    registry: (app as UmbrelApp & { registry?: string }).registry,
    source: 'umbrel',
    original: app,
  });

  // Convert back to UmbrelApp for install modal
  const toUmbrelApp = (app: NormalizedApp): UmbrelApp => {
    return app.original as UmbrelApp;
  };

  // Filter apps by registry
  const apps = appsData?.apps || [];
  const filteredByRegistry = selectedRegistry
    ? apps.filter(app => (app as UmbrelApp & { registry?: string }).registry === selectedRegistry.id)
    : apps;

  // Get categories from API or derive from apps
  ...
  }, [categoriesData, filteredByRegistry]);
